...
def generate_spec_text_from_twin(twin: DigitalTwinResponse) -> bytes:
    ...
    lines.append(
        f"WATER BOOSTER SET - {twin.load_count} PUMPS ({redundancy_str}) - {twin.config_id}"
    )
    ...
    lines.append(
        f"2. The solution shall support {redundancy_str} and cascade PID pressure control using a discharge pressure feedback signal."
    )
    lines.append(
    ...
    lines.append(
        "3. Harmonic mitigation (line reactors, filters, or equivalent) and EMC measures shall be included where required by grid code, EMC requirements, or cable length constraints."
    )
    lines.append("")
    ...
    if comm_protocol.lower() == "no":
        lines.append(
            f"{next_idx}. Each {device_type} shall support hardwired I/O for start/stop, speed reference, and monitoring."
        )
        lines.append(
            f"{next_idx + 1}. Each {device_type} shall provide access to at least the following data via analog/digital signals: run status, fault status, and feedback."
        )
    else:
        lines.append(
            f"{next_idx}. Each {device_type} shall support Ethernet communications using {comm_protocol} for start/stop, speed reference, and monitoring."
        )
        lines.append(
            f"{next_idx + 1}. Each {device_type} shall provide access to at least the following data: run status, feedback, current, power, energy, fault status, and fault code."
        )
    lines.append("")
    ...
    lines.append(
        f"1. The panel shall include {controller_text} with sufficient capacity and I/O to implement required controls."
    )

    if has_scada:
        lines.append(
            "2. The panel shall include a local HMI and a network uplink to a supervisory SCADA system for remote monitoring and control."
        )
    else:
        lines.append(
            "2. The panel shall include a local HMI for operator control and monitoring (start/stop, setpoints, status, alarms, runtime)."
        )

    lines.append(
        "3. The control system shall support a redundant discharge pressure transmitter. Upon primary signal fault/out-of-range, control shall automatically switch to the redundant transmitter and alarm.\n"
    )
    ...
    if comm_protocol.lower() == "no":
        ...
        lines.append(
            f"1. Control and monitoring signals between {(controller_master_text.lower()).replace('the ', '')} and motor starters shall be hardwired via discrete and analog I/O."
        )
        lines.append(
            "2. The panel shall include sufficient terminal blocks to land all interposing hardwired control links."
        )
        lines.append(
            "3. Loss of critical hardwired permissives (e.g., E-Stop, pressure switch) shall transition the system to a defined safe state."
        )
    else:
        lines.append(f"G. COMMUNICATIONS - {comm_protocol.upper()}")
        lines.append(
            f"1. {controller_master_text} shall act as client/master and each motor starter shall act as a server/slave over {comm_protocol}."
        )
        lines.append(
            "2. Each networked device shall have a unique IP address; an IP plan shall be provided."
        )
        lines.append(
            "3. Loss of communications shall generate an alarm and the system shall transition to a defined safe state."
        )
    lines.append("")
    ...
    lines.append(
        "3. Remote access (if required) shall use secure methods with MFA and audit trail; direct internet access to the control network shall not be permitted."
    )
    lines.append(
        "4. Security-relevant events (login attempts, configuration changes, comms loss) shall be logged."
    )
    lines.append("")
    ...
    lines.append(
        "1. The supplier shall provide electrical drawings (SLD, schematics, GA, terminal schedule), I/O list, network IP plan, PLC/HMI backups, and operating manuals."
    )
    lines.append(
        "2. FAT shall verify wiring, labeling, communications, HMI, and core control sequences including staging/de-staging, alternation, and redundancy behavior."
    )
    lines.append(
        "3. SAT shall verify field wiring, motor rotation, transmitter scaling, PID stability, and redundancy in live operation."
    )
