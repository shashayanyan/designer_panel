...
@router.get("/enclosure-options/{pump_count}/{motor_start}/{motor_power}", response_model=Dict[str, str])
def get_enclosure_options_for_config(db: Session = Depends(get_db), pump_count: int = None, motor_start: str = None, motor_power: float = None):
    ...
    config_line = db.query(models.Configuration).filter_by(
        load_count=pump_count,
        series_id=motor_start,
        rated_load_power_kw=motor_power).first()
    if not config_line:
