...
const CircuitBreaker = ({ x, y, label }) => (
  <g transform={`translate(${x}, ${y})`}>
    <text x="25" y="15" fontSize="10" fill="#475569" fontFamily="monospace">
      {label}
    </text>
    <line
      x1="-10"
...
const StarterBlock = ({ x, y, type }) => {
  ...
  if (type === "VSD" || type === "Variable Speed Drive") {
    ...
    symbol = (
      <text x="0" y="24" textAnchor="middle" fontSize="12" fill="currentColor">
        Y/Δ
      </text>
    );
  } else if (type === "") {
...
function BoosterSetPage() {
  ...
  const CURRENT_CONFIG_OPTIONS = useMemo(
    ...
    [
      dynamicMotorStartOptions,
      dynamicMotorPowerOptions,
      dynamicEnclosureOptions,
    ],
  );
    ...
    try {
      const renderSvgToPng = async (svgElement) => {
        ...
        return new Promise((resolve) => {
          ...
          const svgBlob = new Blob([rawSvgData], {
      ...
      if (!response.ok) {
        alert(
          "Failed to generate package. Valid configuration might not exist for these parameters.",
        );
        return;
      }
  ...
  const busLines = [
    busYStart,
    busYStart + busSpacing,
    busYStart + busSpacing * 2,
    busYStart + busSpacing * 4,
  ];
    ...
    <div className="booster">
      <header className="booster__header fade-in">
        ...
        <div className="booster__header-right">
          <button className="btn-secondary" onClick={() => navigate("/water")}>
            ← Back to Water Apps
          </button>
          ...
          <section className="booster__diagram glass-card fade-in fade-in-delay-3">
            ...
            <div className="booster__diagram-canvas">
              ...
              >
                ...
                {hasSCADA && (
                  <image
                    href="/images/booster-set/SCADA.png"
                    x="400"
                    y="30"
                    width="100"
                    height="80"
                ...
                {Array.from({ length: pumpCount || 2 }).map((_, i) => {
                  ...
                  const starterImg = getStarterImage2(
                    config.motorStart,
                    config.motorPower,
                  );
